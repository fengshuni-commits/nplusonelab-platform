...
function pickSellerName(el, text, links) {
    const directCandidates = uniqueTexts([
        ...
        ...collectTextCandidates(el, 'a, span, div')
            .filter(t => /旗舰店|企业店|专卖店|专营店|店铺|商店/.test(t) && t.length <= 60)
    ]).filter(t => !/订单号|官方客服|联系卖家|更多|删除订单|申请开票|查看物流/.test(t));
    ...
    return (links.find(a => /旗舰店|企业店|专卖店|专营店|店铺/.test(a.text))?.text || '').slice(0, 100);
}

function pickProductTitle(el, text) {
    const blocked = /订单号|实付款|更多|查看物流|确认收货|删除订单|申请开票|开票|卖了换钱|联系卖家|官方客服|订单详情|再次购买|退款/;
    const titleCandidates = uniqueTexts([
        ...
        ...collectTextCandidates(el, 'a, span, div')
    ]).filter(t => t && t.length >= 4 && t.length <= 180 && !/^\d+$/.test(t) && !blocked.test(t));

    const preferred = titleCandidates.filter(t => /[\u4e00-\u9fa5A-Za-z]/.test(t) && !/旗舰店|企业店|专卖店|专营店|店铺/.test(t));
    const sorted = (preferred.length ? preferred : titleCandidates).sort((a, b) => {
        const aScore = (/【|】|\(|\)|（|）/.test(a) ? 1 : 0) + (a.length > 12 ? 1 : 0);
        const bScore = (/【|】|\(|\)|（|）/.test(b) ? 1 : 0) + (b.length > 12 ? 1 : 0);
...
function normalizeInvoiceStatus(text, hasDownloadLink) {
    ...
    if (/可开票|申请发票|去开票|开票/.test(normalized)) return '可开票';
    return '未知';
}
